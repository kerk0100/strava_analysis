...
def filtering():
    if request.method == "POST":
        ...
        results = distance(int(dist), str(a_type), str(operand), metrics, start_date, end_date)
        activities_filtered = []
        for a in results:
            activities_filtered.append(a)
        return render_template("filtering.html", column_names=results.columns.values,
                               row_data=list(results.values.tolist()), zip=zip)
